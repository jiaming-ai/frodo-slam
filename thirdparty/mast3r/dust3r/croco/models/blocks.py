...
try:
    ...
    print("FlashAttention is not available. Using standard attention instead.")
...
class Attention(nn.Module):
    ...
    def forward(self, x, xpos):
        ...
        # Check if we can use flash attention (fp16 or bf16)
        can_use_flash = use_flash_attn and (x.dtype == torch.float16 or x.dtype == torch.bfloat16)
        
        if can_use_flash:
            # Reshape for flash_attn_func which expects (batch_size, seqlen, nheads, head_dim)
...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key, value, qpos, kpos):
        ...
        # Check if we can use flash attention (fp16 or bf16)
        can_use_flash = use_flash_attn and (query.dtype == torch.float16 or query.dtype == torch.bfloat16)
        
        if can_use_flash:
            # Reshape for flash_attn_func which expects (batch_size, seqlen, nheads, head_dim)
            q = q.transpose(1, 2)  # B, Nq, num_heads, head_dim
            k = k.transpose(1, 2)  # B, Nk, num_heads, head_dim
            v = v.transpose(1, 2)  # B, Nv, num_heads, head_dim
