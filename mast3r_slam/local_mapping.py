...
from mast3r_slam.frame import KeyframesCuda
...
class FactorGraph:
    def __init__(self, model, frames: KeyframesCuda, K=None, device="cuda"):
        self.model = model
        self.frames = frames
        self.device = device
    ...
    def remove_factors_i(self,i):
        """Remove factors that contain idx i"""
        for k in list(self.factors.keys()):
            if i == k[0] or i == k[1]:
                self.factors.pop(k)
    ...
    def add_factors(self, ii, jj):
        ...
        invalid_edges = torch.minimum(match_frac_j, match_frac_i) < self.cfg["min_match_frac"]
        ...
        n = len(ii_tensor)
        for idx in range(n):
            # replace the old factor if it exists
            self.factors[(ii_tensor[idx].item(), jj_tensor[idx].item())] = (
                idx_i2j[idx], 
                idx_j2i[idx], 
                valid_match_j[idx], 
                valid_match_i[idx], 
    ...
    def get_unique_kf_idx(self, ii, jj):
    ...
    def prep_two_way_edges(self):
        ...
        for k, v in self.factors.items():
            ii_list.append(k[0])
            jj_list.append(k[1])
            idx_ii2jj_list.append(v[0])
            idx_jj2ii_list.append(v[1])
            valid_match_j_list.append(v[2])
            valid_match_i_list.append(v[3])
            Q_ii2jj_list.append(v[4])
            Q_jj2ii_list.append(v[5])

        ii = torch.tensor(ii_list, device=self.device)
        jj = torch.tensor(jj_list, device=self.device)
        idx_ii2jj = torch.stack(idx_ii2jj_list + idx_jj2ii_list, dim=0)
        valid_match = torch.stack(valid_match_j_list + valid_match_i_list, dim=0)
        Q_ii2jj = torch.stack(Q_ii2jj_list + Q_jj2ii_list, dim=0)
    ...
    def solve_GN_rays(self, init=False):
        pin = self.cfg["pin"]

        ii, jj, idx_ii2jj, valid_match, Q_ii2jj = self.prep_two_way_edges()

        unique_kf_idx = self.get_unique_kf_idx(ii,jj)
        ...
        sigma_point = self.cfg["sigma_point"]
        ...
        # mast3r_slam_backends.gauss_newton_points(
        #     pose_data,
        #     Xs,
        #     Cs,
        #     ii,
        ...
        if init:
            self.frames.update_T_WCs(T_WCs, unique_kf_idx)
            # opt all if init is True
            return T_WCs, unique_kf_idx
        else:
            self.frames.update_T_WCs(T_WCs[pin:], unique_kf_idx[pin:])
            return T_WCs[pin:], unique_kf_idx[pin:]
