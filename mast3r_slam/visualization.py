...
class Window(WindowEvents):
    ...
    def set_keyframes(self, new_keyframes):
        ...
        self.textures.clear()  # This clears all point textures
        self.frustums = Frustums(self.line_prog)
        self.lines = Lines(self.line_prog)
        
        # Reset cached point projection data
        self.dP_dz = None
        
        # Reset any other state that might affect point rendering
        self.curr_img_np, self.kf_img_np = None, None
        self.curr_img, self.kf_img = Image(), Image()
    ...
    def render(self, t: float, frametime: float):
        ...
        for kf_idx in range(N_keyframes):
            keyframe = self.keyframes.get_frame_cpu(kf_idx)
            # for rare cases, keyframe is not in the textures
            if keyframe.frame_id not in self.textures:
                continue
            h, w = keyframe.img_shape.flatten()
...
def run_visualization(cfg, states, keyframes, main2viz, viz2main) -> None:
    ...
    while not window.is_closing:
        # Check for messages from main process with better error handling
        try:
            msg = main2viz.get_nowait()
            if isinstance(msg, dict) and "new_keyframes" in msg:
                print("Received new keyframes")
                window_config.set_keyframes(msg["new_keyframes"])
