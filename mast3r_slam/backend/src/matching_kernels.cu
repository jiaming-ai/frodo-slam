...
__global__ void refine_matches_kernel(
    ...
    torch::PackedTensorAccessor32<scalar_t,2,torch::RestrictPtrTraits> max_scores,
    const int radius,
    ...
    )
{
  ...
  // Store the max score instead of validity
  max_scores[b][n] = max_score;
}
    ...
    const int dilation)
{
  ...
  // Create max_scores tensor with same dtype as D11/D21
  auto opts_scores = opts.dtype(D11.scalar_type());
  torch::Tensor max_scores = torch::zeros(
    {batch_size, n}, opts_scores);
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(D11.scalar_type(), "refine_matches_kernel", ([&] {
    refine_matches_kernel<scalar_t><<<blocks, threads>>>(
      ...
      max_scores.packed_accessor32<scalar_t,2,torch::RestrictPtrTraits>(),
      radius,
      dilation
    );
   }));

  return {p1_new, max_scores};
}
